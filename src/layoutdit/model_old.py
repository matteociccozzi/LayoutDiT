...
class LayoutDetectionModel(nn.Module):
    def __init__(
        ...
        self.fpn_out_channels = hidden_dim

        # 2) 4‑level FPN (P2–P5) from one base map
        #    We will generate P2 (14×14), P3 (7×7), P4 (4×4), P5 (2×2) by pooling
        self.fpn = FeaturePyramidNetwork(
            in_channels_list=[hidden_dim] * 4,
            out_channels=self.fpn_out_channels,
        )

        self.classifier = nn.Linear(self.fpn_out_channels, num_classes + 1)
        self.bbox_regressor = nn.Linear(self.fpn_out_channels, 4)
    ...
    def forward(self, images):
        ...
        batch_size, patches, categories = patch_feats.shape
        grid = int(patches ** 0.5) # ok because 224x224 images

        # Reshape to 2D feature map: [B, C, H', W']
        x = patch_feats.permute(0, 2, 1).view(batch_size, categories, grid, grid)

        # Apply FPN: expects an OrderedDict of feature maps
        p2 = x  # 14×14
        p3 = F.adaptive_avg_pool2d(x, output_size=(grid // 2, grid // 2))  # 7×7
        p4 = F.adaptive_avg_pool2d(x, output_size=(grid // 4, grid // 4))  # ~4×4
        p5 = F.adaptive_avg_pool2d(x, output_size=(grid // 8, grid // 8))  # ~2×2

        # feed into FPN
        fpn_inputs = OrderedDict([("p2", p2), ("p3", p3), ("p4", p4), ("p5", p5)])
        fpn_feats = self.fpn(fpn_inputs)
        # fpn_feats is a dict with keys p2–p5, each [B, fpn_out, H_i, W_i]

        # --- for a simple patch‑wise head we can just upsample all levels to p2's size and concat ---
        up_p3 = F.interpolate(fpn_feats["p3"], size=p2.shape[-2:], mode="bilinear", align_corners=False)
        up_p4 = F.interpolate(fpn_feats["p4"], size=p2.shape[-2:], mode="bilinear", align_corners=False)
        up_p5 = F.interpolate(fpn_feats["p5"], size=p2.shape[-2:], mode="bilinear", align_corners=False)

        # concatenate along channels: [B, out*4, grid, grid]
        fused = torch.cat([fpn_feats["p2"], up_p3, up_p4, up_p5], dim=1)

        # reduce back to a single feature dimension per patch
        fused = nn.Conv2d(
            in_channels=self.fpn_out_channels * 4,
            out_channels=self.fpn_out_channels,
            kernel_size=1,
        )(fused)  # [B, fpn_out_channels, grid, grid]

        # flatten back to [B, P, fpn_out_channels]
        patch_feats = fused.flatten(2).permute(0, 2, 1)
